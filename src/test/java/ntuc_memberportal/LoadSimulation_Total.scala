...
class LoadSimulation_Total extends Simulation {
  ...
  setUp(
    user_Service.inject(constantConcurrentUsers(500).during(300)),
    member_Service.inject(constantConcurrentUsers(500).during(300)),
    ...
    payment_Service.inject(constantConcurrentUsers(500).during(300))
  )
}
