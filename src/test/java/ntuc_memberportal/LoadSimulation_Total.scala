...
//  val protection_Service: ScenarioBuilder = scenario("protection-service").exec(karateFeature("classpath:ntuc_memberportal.FeatureFiles.protection-service/case.feature"))



  setUp(
    user_Service.inject(constantConcurrentUsers(5000).during(1500)),
    member_Service.inject(constantConcurrentUsers(5000).during(1500)),
    deal_Service.inject(constantConcurrentUsers(5000).during(1500)),
    notification_Service.inject(constantConcurrentUsers(5000).during(1500)),
    payment_Service.inject(constantConcurrentUsers(5000).during(1500))
