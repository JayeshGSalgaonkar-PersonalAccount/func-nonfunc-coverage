...
class LoadSimulation_Total extends Simulation {
  ...
  setUp(
    user_Service.inject(constantConcurrentUsers(800).during(300)),
    member_Service.inject(constantConcurrentUsers(800).during(300)),
    deal_Service.inject(constantConcurrentUsers(800).during(300)),
    notification_Service.inject(constantConcurrentUsers(800).during(300)),
    payment_Service.inject(constantConcurrentUsers(800).during(300))
  )
}
