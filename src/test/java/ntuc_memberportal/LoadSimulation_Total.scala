...
class LoadSimulation_Total extends Simulation {
  ...
  setUp(
    user_Service.inject(constantConcurrentUsers(800).during(400)),
    member_Service.inject(constantConcurrentUsers(800).during(400)),
    deal_Service.inject(constantConcurrentUsers(800).during(400)),
    notification_Service.inject(constantConcurrentUsers(800).during(400)),
    payment_Service.inject(constantConcurrentUsers(800).during(400))
  )
}
